...
@ConditionalOnProperty(value = "runner.endpoint.FilterAppRunner", havingValue = "true",
        matchIfMissing = true)
public class FilterAppRunner implements ApplicationRunner {

    private final PrinterGateway filterGateway;


    @SneakyThrows
    @Override
    public void run(ApplicationArguments args) {
        log.info("RUNNING FilterAppRunner !!!");
        List<Future<Message<String>>> futures = new ArrayList<>();
        for (int i = 0; i < 10; i++) {
            Message<?> message = getMessage(i);

            log.info("Sending Message: {}", message);
            futures.add(filterGateway.print(message));
        }
