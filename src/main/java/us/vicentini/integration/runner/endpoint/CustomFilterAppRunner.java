...
@ConditionalOnProperty(value = "runner.endpoint.CustomFilterAppRunner", havingValue = "true",
        matchIfMissing = true)
public class CustomFilterAppRunner implements ApplicationRunner {

    private final PrinterGateway customFilterGateway;


    @SneakyThrows
    @Override
    public void run(ApplicationArguments args) {
        log.info("RUNNING CustomFilterAppRunner !!!");
        List<Future<Message<String>>> futures = new ArrayList<>();
        for (int i = 0; i < 10; i++) {
            Message<?> message = getMessage(i);

            log.info("Sending Message: {}", message);
            futures.add(customFilterGateway.print(message));
        }
